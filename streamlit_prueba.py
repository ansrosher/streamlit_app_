...
st.set_page_config(layout="wide", page_title="QuantStrategyAI")

# CSS personalizado inspirado en Robinhood (basado en la imagen)
import streamlit as st

# CSS personalizado inspirado en Robinhood (colores exactos de la imagen)
st.markdown("""
    <style>
    ...
    /* Main App Background */
    .stApp {
        background-color: #D4FF00;  /* Bright yellow-green background */
    ...
    /* Sidebar */
    .css-1d391kg {
        background-color: #000000;  /* Black background from navigation bar */
        border-right: 1px solid #333333;
        padding-top: 0;
    }
    .sidebar-title {
        color: #FFFFFF;            /* White text on black sidebar */
        font-size: 24px;
        padding: 1rem;
    ...
    h2 {
        color: #666666;           /* Medium gray for subheadings */
        font-weight: 400;
    }
    
    /* Sidebar Section Text */
    .section-title {
        color: #FFFFFF;           /* White for sidebar text */
        font-size: 18px;
        font-weight: 400;
...
def submenu_portafolio_minima_varianza():
    with st.container():
        ...
        if st.button("Optimizar Portafolio"):
            ...
            apds = [
                mpf.make_addplot(normalized_weights, type='bar', width=0.35, 
                                color='blue', label='Mínima Varianza (ajustado)'),
            ]
            ...
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
                # Create the plot
                mpf.plot(
                    data,  # Note: data should be a DataFrame with index matching significant_tickers
                    type='bar',
                    style='yahoo',
                    addplot=apds,
                    title='Pesos en el Portafolio Ajustado (pesos >= 1%)',
                    ylabel='Pesos',
                    xlabel='Activos',
                    show_nontrading=False,
                    savefig=tmp.name,
                    # Custom settings for bar chart
                    mav=None,  # Disable moving averages
                    panel_ratios=(1,0),  # Single panel
                )
                tmpfile_path = tmp.name
            ...
            # Prepare additional plots
            apds = [
                mpf.make_addplot(min_var_cumulative_adjusted, color='blue', label='Portafolio de Mínima Varianza (ajustado)'),
                mpf.make_addplot(equal_weighted_cumulative, color='orange', label='Portafolio Equally Weighted'),
            ]
            
            # Create temporary file
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
                # Plot with additional indicators
                mpf.plot(
                    data,  # Note: you'll need to ensure your data is in the correct format (OHLC)
                    type='line',  # Changed from candle to line since it's portfolio value
                    style='yahoo',
                    addplot=apds,
                    title='Evolución del Valor del Portafolio',
                    ylabel='Valor del Portafolio (normalizado)',
                    show_nontrading=False,
                    savefig=tmp.name
                )
